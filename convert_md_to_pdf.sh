...
for file in $(find report -type f -name '*.md'); do
  ...
  mkdir -p "$output_dir"
  pandoc -f gfm -t html5 --metadata pagetitle="file.md" -s --css="https://gist.githubusercontent.com/killercup/5917178/raw/40840de5352083adb2693dc742e9f75dbb18650f/pandoc.css" --from markdown+raw_attribute+link_attributes+citations "$file" -o "$output_dir/$output_file"
